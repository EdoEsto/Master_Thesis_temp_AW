### Note: This work is the creation and original work of Bert Wang-Chak Chan with slight modifications outlines in my thesis hand-in.
    ...
    def transform(self, tx, mode='RZSF', z_axis=Z_AXIS, is_world=False):
        if 'R' in mode and tx['rotate'] != [0]*3:
            if DIM == 2:
                ...
                self.cells = scipy.ndimage.rotate(self.cells, tx['rotate'][2], axes=(X_AXIS, z_axis), reshape=not is_world, order=3, mode='wrap' if is_world else 'constant')  # rotate by y axis = x-z plane
                self.cells = scipy.ndimage.rotate(self.cells, tx['rotate'][1], axes=(z_axis, Y_AXIS), reshape=not is_world, order=3, mode='wrap' if is_world else 'constant')  # rotate by x axis = z-y plane
                self.cells = scipy.ndimage.rotate(self.cells, tx['rotate'][0], axes=(Y_AXIS, X_AXIS), reshape=not is_world, order=3, mode='wrap' if is_world else 'constant')  # rotate by z axis = y-x plane
...
class Lenia:
    ...
    def save_world(self, is_seq=False):
        ...
        try:
            ...
            if is_seq:
                ...
                self.file_seq = 0
            path = os.path.join(self.SAVE_ROOT, str(self.animal_id) + '_' + str(self.file_seq))
    ...
    def run(self):
        counter = 0
        while self.is_loop:
            if self.specific_save > 0 and self.file_seq < 5000:
                self.save_world(is_seq=True)
            counter += 1
            if self.last_key:
